...
export default function SessionList({
  ...
  return (
    <aside className="flex min-h-0 flex-1 flex-col border-r border-border bg-elevated">
      ...
      {/* Filter bar outside the scroll regions; as a flex item its z-10
          keeps open dropdowns painting over the regions below. */}
      <div className="z-10 flex h-10 shrink-0 items-center justify-start gap-2 border-b border-border bg-elevated px-3">
        {showHostInfo && (
          <HostFilterDropdown
            hosts={uniqueHosts}
      ...
      {workspaceView && !loading ? (
        <div
          ref={navigatorBodyRef}
          className="flex min-h-0 flex-1 flex-col"
          data-testid="workspace-navigator"
        >
          {/* Flow region: change/worktree sections scroll together. Its
              min-height floor makes the panes shrink proportionally first
              when the navigator is too short for every default. */}
          <div
            className="flex-1 overflow-y-auto"
            style={{ minHeight: FLOW_REGION_MIN_HEIGHT }}
            data-testid="workspace-flow-region"
          >
