...
**8. Runner selection rolled back to GitHub-hosted runners for this public repository (2026-09-14, during validation).**
The pool evaluation did exactly what decision 4 designed — it named what the image lacked — and the list was decisive: no C toolchain (node-pty's node-gyp build fails with `not found: make`), no tmux, no Chromium system libraries, and no passwordless sudo, so no workflow step could close the gaps; serving this repo locally meant building and maintaining a second runner image. `b497047` had already surfaced an infrastructure constraint: organization-scoped scale-set names are unique per org, so the public-repository pool had to be `local-k8s-public` — a deliberately weaker pool (no BuildKit client cert, public registries, ≤2 runners, egress denied to private address space) because fork PRs can run code on it. Since kawai is public, GitHub-hosted standard runners are free and unlimited and keep fork-PR code off the cluster node entirely; `875b924` moved all four workflows to `ubuntu-latest` (merged in PR #1), and the weakened public pool with its egress NetworkPolicy can be retired. What survives from this change: decisions 2 (from hosted runners the proxy probe always misses and falls back with the designed warning), 3, 4, 6, and 7 in full, plus decision 1's timeouts and concurrency — only the runner *label* is gone. Alternative considered — build a second runner image carrying the full dependency set — rejected: permanent image maintenance for zero cost benefit on a public repo.
...
1. Dispatch `local-runner-test.yml`; fix image gaps it names (cluster-side) until green. — *Outcome: the gaps were named (make, tmux, Chromium system libraries, no sudo); fixing them meant maintaining a second runner image — rejected.*
2. Land workflow changes; open a throwaway PR to watch `ci.yml` + e2e run on the pool end to end. — *Outcome: the pool runs acted as the designed diagnosis; PR #1 landed the pivot (`b497047` → `local-k8s-public`, then `875b924` → hosted `ubuntu-latest`) together with the e2e rebrand fix (`27152d6`).*
3. Cut a canary tag (patch bump) to exercise the cross-compiled release chain; verify the GitHub Release carries the four tarballs and the darwin binaries run on a real Mac before announcing. — *Outcome: `v0.5.2`/`v0.5.3` published 2026-09-14, each carrying all four platform binaries + tarballs (GitHub API verified 2026-09-15); npm's latest remains the pre-change `0.5.2` (published 2026-09-10) and the tap was untouched; the real-Mac run of the darwin binary is the operator's acceptance check on `v0.5.3`.*
4. Rollback: revert the single workflows commit — hosted runners resume exactly as before; no cluster-side state is affected. — *Outcome: the runner-label portion was rolled back this way by `875b924`; the hardening (timeouts, concurrency, setup action, cross-compile, releases-only publishing) stayed.*
